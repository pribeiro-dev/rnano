...
# Bootstrap the GitHub project for rnano: milestones, labels, issues, and an
# optional Project board.
#
# Usage: run from the repository root after pushing to GitHub.
# Requires: gh (https://cli.github.com/) authenticated, and jq.
#
# Optional environment overrides:
#   REPO=owner/name           Target repo (autodetected from the git remote)
#   PROJECT_TITLE="..."       Project board title (default: "rnano Roadmap")
#   PROJECT_NUMBER=N          Reuse an existing project instead of creating one
#   SKIP_PROJECT=1            Skip Project board creation entirely
...
# --- (1) Project board (optional) ---
PROJECT_TITLE=${PROJECT_TITLE:-"rnano Roadmap"}
PROJECT_NUMBER=${PROJECT_NUMBER:-}

ensure_project() {
  if [[ -n "${SKIP_PROJECT:-}" ]]; then
    say "Skipping project board (SKIP_PROJECT set)"
    return
  fi
  ...
  say "Created project #${PROJECT_NUMBER}"
}
...
# --- (5) Add open issues to the Project board ---
add_all_issues_to_project() {
  [[ -z "${PROJECT_NUMBER}" ]] && return
  say "Adding open issues to project #${PROJECT_NUMBER}"
  mapfile -t issue_urls < <(gh issue list --repo "$REPO" --state open --limit 500 --json url -q '.[].url')
...
"Matrix os/toolchain; cargo steps for fmt/clippy/build/test."
